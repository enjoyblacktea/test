...
### Requirement: Display typed shuangpin keys progressively in shuangpin mode
系統必須（SHALL）在雙拼模式下，在輸入提示區域逐步顯示使用者已按下的字母鍵。

#### Scenario: User types first shuangpin key correctly
- **WHEN** 使用者在雙拼模式下按下正確的聲母鍵（第一鍵）
- **THEN** 系統在輸入顯示區域顯示第一個字母
- **THEN** 畫面反映已輸入 1/2 鍵的狀態

#### Scenario: User completes shuangpin input
- **WHEN** 使用者按下正確的韻母鍵（第二鍵）
- **THEN** 系統在輸入顯示區域顯示完整的兩字母碼
- **THEN** 系統隨即自動載入下一個練習字並清空顯示
