...
The system SHALL provide visual feedback by highlighting the corresponding key when the user presses a keyboard key, using high-contrast cyan color that matches the dark theme. In shuangpin mode, the highlighted key is on the QWERTY layout; in zhuyin mode, the highlighted key is on the zhuyin layout.

#### Scenario: User presses valid zhuyin key
- **WHEN** user presses a key that maps to a zhuyin symbol in zhuyin mode
- **THEN** the system highlights the corresponding virtual keyboard key with cyan color (#00d9ff)
- **THEN** the highlight is visible for a minimum of 150ms
- **THEN** the highlight includes a subtle scale transform animation

#### Scenario: User presses valid letter key in shuangpin mode
- **WHEN** user presses a letter key in shuangpin mode
- **THEN** the system highlights the corresponding QWERTY key with cyan color (#00d9ff)
- **THEN** the highlight is visible for a minimum of 150ms
- **THEN** the highlight does not indicate correct or incorrect (neutral highlight only)

#### Scenario: User presses invalid key
- **WHEN** user presses a key that does not map to any symbol in the current mode
...
### Requirement: VirtualKeyboard accepts layout prop to switch between zhuyin and QWERTY
The system SHALL support a `layout` prop on the `VirtualKeyboard` component to render either the zhuyin symbol layout or the QWERTY dual-label layout depending on the current input method mode.

#### Scenario: Zhuyin layout rendered
- **WHEN** VirtualKeyboard is rendered with layout="zhuyin"
- **THEN** the keyboard displays the bopomofo symbol arrangement
- **THEN** the behavior is identical to existing zhuyin keyboard

#### Scenario: QWERTY layout rendered with dual labels
- **WHEN** VirtualKeyboard is rendered with layout="shuangpin"
- **THEN** the keyboard displays QWERTY rows (qwertyuiop / asdfghjkl / zxcvbnm)
- **THEN** each key shows an uppercase letter as the primary label
- **THEN** each key shows the corresponding Xiaohe shuangpin bopomofo initial or final as a secondary label using zhuyin characters (ㄅㄆㄇ), not pinyin letters
- **THEN** keys that serve as both initial and final display both mappings (e.g. N → ㄋ / ㄧㄠ)
