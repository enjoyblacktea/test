...
export function init(deps = {}) {
    // particleSystem = deps.particleSystem; // REMOVED: Particle system removed for dark theme
...
async function handleKeyDown(event) {
    ...
    if (result.correct) {
        ...
        // Trigger correct-pulse animation on practice character
        if (animationController && !result.complete) {
            const charElement = document.getElementById('practice-character');
            if (charElement) {
                animationController.triggerCorrectPulse(charElement);
            }
        }
        ...
        if (result.complete) {
            ...
            if (animationController) {
                const cardElement = document.querySelector('.practice-card');
                if (cardElement) {
                    animationController.triggerCompletionGlow(cardElement);
                }
            }
